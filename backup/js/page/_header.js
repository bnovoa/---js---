define(function(require) {
	...
	$(function() {
		...
		magicController.getUserInfo(function(data) {
			if (data.ret == 1) {
				...
				$nologin.hide();
				$islogin.show();
				$('#account_name').replaceWith("<a href='user-center.html'>" + data.user.email + "</a>");
			}
		});
		/*var account = Cookie.getCookie("account");
		var passwd = Cookie.getCookie("_pd");
		var token = Cookie.getCookie("token");
		var platform = Cookie.getCookie("platform");
	...
	function loginout() {
		$.post(url + '/h/v2/user/logout', function(data) {
			if (data.ret == 1) {
				...
				Cookie.removeCookie("_pd");
				// Cookie.removeCookie("JSESSIONID");
	...
	function register_submit() {
		...
		var password = Md5.hex_md5($("#set-passward").val());
		$.ajax({
			cache: false,
			type: "post",
			...
			success: function(obj) {
				...
				if (ret == 1) {
					...
					// Cookie.setCookie("_pd", Md5.hex_md5($('#set-passward').val()));
					closeWin();
					magicController.sendEmailRepo(email,"注册");
				} else {
					closeWin();
	...
	function login_submit() {
		...
		var _passwd = Cookie.getCookie("_pd");
		...
		$.post(url + '/h/v2/user/login', {
			account: account,
			passwd: passwd==_passwd?passwd:Md5.hex_md5(passwd)
		}, function(data) {
			var ret = data.ret;
			// console.log("------------>")
			// console.log(ret);
			...
			if (ret == 1) {
				if(recDays == 14){
					...
					Cookie.setCookie("_pd", Md5.hex_md5(passwd), recDays);
				}else{
					Cookie.removeCookie("account");
					Cookie.removeCookie("_pd");
				}
...
*/
	...
	function showLoginWin() {
		var _account = Cookie.getCookie("account");
		var _passwd = Cookie.getCookie("_pd");
		if (_account != null && _passwd !== null) {
			$('#login-account').val(_account);
			$('#login-psw').val(_passwd);
		} else {
			resetForm();
		}
