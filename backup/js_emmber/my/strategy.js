...
function post_list_page(userId, page)
{
	...
	var strategyType = $("#select_type").val();
	http("../forumManage/strategyListLimit",
	{
		userId : userId,
		page : page,
...
function get_post_id_page(userId, postId, page)
{
	...
	}, function(data)
	{
		...
		$("#select_strategy_type").val(post.strategyType);
...
function save_strategy(userId)
{
	var postId = $("#td_post_id").html();
	var strategyType = $("#select_strategy_type").val();
	http("../forumManage/saveStrategyLimit",
	{
		userId : userId,
		postId : postId,
		strategyType : strategyType
