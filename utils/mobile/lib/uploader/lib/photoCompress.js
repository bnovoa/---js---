//GJ.add('js/app/mobile/uploader/lib/photoCompress.js', [], function(){});
GJM=GJM||{};
GJM.lib=GJM.lib||{};
GJM.lib.ImageCompresser = (function(lib) {
    var $ = window.$;
    
    var JpegMeta = lib.JpegMeta;
    var JPEGEncoder = lib.JPEGEncoder;
    var ImageCompresser = {isIOSSubSample: function(img) {
            ...
            if (w * h > 1024 * 1024) {
                ...
                hasSubSample = !!(ctx.getImageData(0,
                0, 1, 1).data[3] === 0);
                canvas = ctx = null
            }
            return hasSubSample
        ...
        param) {
            ...
            var orienFix = this.acfix({width: orienW,height: orienH}, {width: maxW,
                height: maxH});
            ...
            if ($.os.iphone || $.os.ipad) {
                ...
                var vertSquashRatio =
                ImageCompresser.getIOSImageRatio(img, naturalW, naturalH);
                var sy = 0;
                while (sy < naturalH) {
                    if (sy + d > naturalH)
                        var sh = naturalH - sy;
                    ...
                    while (sx < naturalW) {
                        ...
                        ctx.drawImage(tmpCanvas, 0, 0, sw, sh, dx, dy,
                        dw, dh);
                        sx += d;
                    }
                    sy += d;
    ...
    return {getImageData: getImageData}
})(GJM.lib);