define(function(require) {
  ...
  $("#footer_div").load("_footer.html",function () {
	   ...
	   if(error_code != 0){
		...
		$.ajax({
				...
				success:function(data){
					...
					if(ret == 1){
					   $("#findpsw_1").hide();
					   $("#findMsg").html("系统已发送邮件到<font style='color:#ff6511;font-weight:bold;'>"+$('#findByEmail').val()+"</font>，请点击邮件的链接重置你的密码");
					   $("#findpsw_2").show();
					}else{
	...
	$('#findByEmail').focus(function(){
	    ...
	    $.post(url+'/h/v2/user/validAccount', {account : $('#findByEmail').val()}, function(data) {
	    	console.log(data);
			var data = eval('('+data+')');																						 
			if (data.ret != 1){
				error_code = -9;
	...
	$('#findByPhone').focus(function(){
		...
		clearErrorMsg(-1);
	    $.post(url+'/h/v2/user/validAccount', {account : $('#findByPhone').val()}, function(data) {
