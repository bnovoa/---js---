...
/* 		magicController.showUserBuyInfo("#proList"); */
		$(".buy-submit").click(function(event){
			...
			magicController.createOrder(params,decodeURI(cartJsonStr));
		...
		$("#valid_button").click(function(event){
			...
			// 去掉原按钮，并插入loading图			
			$btn.hide();
			$textField.prop({disabled: true}).after($loading);
    ...
    $('#row-invoice [name=oisNeedReceipt]').change(function() {
      ...
      var $companyRow = $container.find('.company-row').hide();
		...
		var addressWhiteList = [
			{name:'北京', cityList:['市辖区','县']},
			{name:'上海', cityList:['市辖区','县']},
    	...
    	//saveAddress 
    	magicController.getExistsAddress(function(data){
    		// console.log("---------->已存在的地址<-------------");
    		// console.log(data);
    		var address;
